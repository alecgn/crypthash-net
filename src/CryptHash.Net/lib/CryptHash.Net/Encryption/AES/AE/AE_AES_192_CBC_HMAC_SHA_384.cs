...
namespace CryptHash.Net.Encryption.AES.AE
{
    public class AE_AES_192_CBC_HMAC_SHA_384 : AesBase
    {
        ...
        /// <summary>
        /// Encrypts an input string using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided password string.
        /// </summary>
        ...
        /// <summary>
        /// Encrypts an input string using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided SecureString with the password.
        /// </summary>
        /// <param name="plainString">The input plain string to encrypt.</param>
        ...
        /// <summary>
        /// Encrypts an input byte array of the string using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided SecureString with the password.
        /// </summary>
        ...
        /// <summary>
        /// Encrypts an input byte array of the string using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided byte array of the password.
        /// </summary>
        /// <param name="plainStringBytes">The input byte array of the string to encrypt.</param>
        ...
        public AesEncryptionResult EncryptString(byte[] plainStringBytes, byte[] passwordBytes, bool appendEncryptionDataToOutput = true)
        {
            ...
            try
            {
                ...
                if (aesEncryptionResult.Success)
                {
                    ...
                    if (appendEncryptionDataToOutput)
                    {
                        using (var ms = new MemoryStream())
                        {
                            using (var bw = new BinaryWriter(ms))
                            {
                                ...
                                //hmacSha384bytes = CommonMethods.ComputeHMACSHA384HashFromDataBytes(authKey, encryptedData, 0, encryptedData.Length);
                                hmacSha384bytes = new HMAC_SHA_384().ComputeHMAC(encryptedData, authKey, 0, encryptedData.Length).HashBytes;
                    ...
                    {
                        //hmacSha384bytes = CommonMethods.ComputeHMACSHA384HashFromDataBytes(authKey, aesEncryptionResult.EncryptedDataBytes, 0, aesEncryptionResult.EncryptedDataBytes.Length);
                        hmacSha384bytes = new HMAC_SHA_384().ComputeHMAC(aesEncryptionResult.EncryptedDataBytes, authKey, 0, aesEncryptionResult.EncryptedDataBytes.Length).HashBytes;
                        tag = hmacSha384bytes.Take(_tagBytesLength).ToArray();
        ...
        /// <summary>
        /// Encrypts an input byte array of the string using AES with a 192 bits key in CBC mode with HMACSHA384 authentication.
        /// </summary>
        /// <param name="plainStringBytes">The input byte array of the string to encrypt.</param>
        /// <param name="encryptionKey">The byte array of the 192 bit encryption key used to encrypt data. Leave empty or pass null to auto-generate a secure random 192 bit key.</param>
        ...
        /// <param name="authenticationKey">The byte array of the authentication key used to generate a tag and authenticate the data during decryption. Leave empty or pass null to auto-generate a secure random 192 bit authentication key.</param>
        /// <returns>AesEncryptionResult</returns>
        public AesEncryptionResult EncryptString(byte[] plainStringBytes, byte[] encryptionKey = null, byte[] IV = null, byte[] authenticationKey = null)
        {
            ...
            try
            {
                ...
                if (aesEncryptionResult.Success)
                {
                    ...
                    hmacSha384HashedBytes = new HMAC_SHA_384().ComputeHMAC(aesEncryptionResult.EncryptedDataBytes, authenticationKey, 0, aesEncryptionResult.EncryptedDataBytes.Length).HashBytes;
                    tag = hmacSha384HashedBytes.Take(_tagBytesLength).ToArray();
                    aesEncryptionResult.Tag = tag;
        ...
        /// <summary>
        /// Decrypts a base64 encoded input string using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided password string.
        /// </summary>
        ...
        /// <summary>
        /// Decrypts a base64 encoded input string using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided password in a SecureString.
        /// </summary>
        /// <param name="base64EncryptedString">The base64 encoded input string to decrypt.</param>
        ...
        /// <summary>
        /// Decrypts a byte array of the string using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided password in a SecureString.
        /// </summary>
        ...
        /// <summary>
        /// Decrypts a byte array of the string using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided byte array of the password.
        /// </summary>
        /// <param name="encryptedStringBytes">The input byte array of the string to decrypt.</param>
            ...
            byte[] sentTag = null, byte[] salt = null, byte[] IV = null)
        {
            ...
            try
            {
                ...
                if (!CommonMethods.TagsMatch(calcTag, sentTag))
                {
                    ...
                    {
                        Success = false,
                        Message = MessageStrings.Decryption_AuthenticationTagsMismatchError
                    };
                }
        ...
        /// <summary>
        /// Decrypts a byte array of the string using AES with a 192 bits key in CBC mode with HMACSHA384 authentication.
        /// </summary>
        /// <param name="encryptedStringBytes">The input byte array of the string to decrypt.</param>
        /// <param name="encryptionKey">The previously generated byte array of the 192 bit encryption key used to encrypt data.</param>
        ...
        /// <param name="authenticationKey">The previously generated byte array of the 192 bit authentication key.</param>
        ...
        public AesDecryptionResult DecryptString(byte[] encryptedStringBytes, byte[] encryptionKey, byte[] IV, byte[] authenticationKey, byte[] sentTag)
        {
            ...
            try
            {
                var hmacSha384Bytes = new HMAC_SHA_384().ComputeHMAC(encryptedStringBytes, authenticationKey, 0, encryptedStringBytes.Length).HashBytes;
                var calcTag = hmacSha384Bytes.Take(_tagBytesLength).ToArray();
        ...
        /// <summary>
        /// Encrypts a file using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided password string.
        /// </summary>
        ...
        /// <summary>
        /// Encrypts a file using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided SecureString with the password.
        /// </summary>
        /// <param name="sourceFilePath">The input source file path to encrypt.</param>
        /// <param name="encryptedFilePath">The output file path to save the encrypted file. Pass null or an empty string to not generate a new file, encrypting only the input source file and mantaining its path.</param>
        /// <param name="secStrPassword">The SecureString with the password where the encryption key will be derived from.</param>
        ...
        /// <summary>
        /// Encrypts a file using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided byte array of the password.
        /// </summary>
        /// <param name="sourceFilePath">The input source file path to encrypt.</param>
        /// <param name="encryptedFilePath">The output file path to save the encrypted file. Pass null or an empty string to not generate a new file, encrypting only the input source file and mantaining its path.</param>
        /// <param name="passwordBytes">The byte array of the password where the encryption key will be derived from.</param>
        ...
        public AesEncryptionResult EncryptFile(string sourceFilePath, string encryptedFilePath, byte[] passwordBytes, bool deleteSourceFile = false, bool appendEncryptionDataToOutputFile = true)
        {
            ...
            try
            {
                ...
                if (aesEncryptionResult.Success)
                {
                    ...
                    {
                        RaiseOnEncryptionMessage(MessageStrings.Encryption_FileAdditionalDataWriting);
                    ...
                    //var hmacSha384 = CommonMethods.ComputeHMACSHA384HashFromFile(encryptedFilePath, authKey);
                    var hmacSha384 = new HMAC_SHA_384().ComputeFileHMAC(encryptedFilePath, authKey).HashBytes;
                    var tag = hmacSha384.Take(_tagBytesLength).ToArray();

                    if (appendEncryptionDataToOutputFile)
                    {
                        CommonMethods.AppendDataBytesToFile(encryptedFilePath, tag);
                        RaiseOnEncryptionMessage(MessageStrings.Encryption_FileAdditionalDataWritten);
                    }
        ...
        /// <summary>
        /// Decrypts a file using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided password string.
        /// </summary>
        ...
        /// <summary>
        /// Decrypts a file using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided SecureString with the password.
        /// </summary>
        /// <param name="encryptedFilePath">The input source encrypted file path do decrypt.</param>
        /// <param name="decryptedFilePath">The output file path to save the decrypted file. Pass null or an empty string to not generate a new file, decrypting only the input source encrypted file and mantaining its path.</param>
        /// <param name="secStrPassword">The SecureString with the password where the encryption key will be derived from.</param>
        ...
        /// <summary>
        /// Decrypts a file using AES with a 192 bits key in CBC mode with HMACSHA384 authentication, deriving the key from a provided byte array of the password.
        /// </summary>
        /// <param name="encryptedFilePath">The input source encrypted file path do decrypt.</param>
        /// <param name="decryptedFilePath">The output file path to save the decrypted file. Pass null or an empty string to not generate a new file, decrypting only the input source encrypted file and mantaining its path.</param>
        /// <param name="passwordBytes">The byte array of the password where the encryption key will be derived from.</param>
